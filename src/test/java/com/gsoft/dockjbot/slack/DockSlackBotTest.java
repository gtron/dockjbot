...
public class DockSlackBotTest {
    ...
    @Test
    public void when_OldStatusByMention_Should_logTimedout() throws Exception {
        
        String expiredTs = "" + ( ( System.currentTimeMillis() - 600000 ) / 1000 );
        ...
        assertThat(capture.toString(), containsString("Timed out"));
        assertThat(capture.toString(), not(containsString("Executed command")));
    }
    
    
    @Test
    public void when_StatusByMention_Should_NotTimedout() throws Exception {
        
        String notExpiredTs = "" + ( ( System.currentTimeMillis() - 1000 ) / 1000 );
        ...
        assertThat(capture.toString(), containsString("Executed command"));
        assertThat(capture.toString(), not( containsString("Timed out") ));
    }
    
    @Test
    public void when_psByMention_Should_Execute_ps() throws Exception {
        ...
        TextMessage textMessage = new TextMessage("{\"type\": \"message\"," +
                "\"ts\": \""+ notExpiredTs  +"\"," +
                "\"user\": \"U023BECGF\"," +
                "\"text\": \"<@UEADGH12S>: "+ command +"\"}");
        bot.handleTextMessage(session, textMessage);
        
        assertThat(capture.toString(), containsString("Executed command: dock " + command ));
        assertThat(capture.toString(), not( containsString("Timed out") ));
    }
...
//    }
     ...
     */
    public static class TestBot extends DockSlackBot {
        ...
        protected StringBuffer execDockCommand(String command, String options) {
            return new StringBuffer("Executed command: "). 
                    append( command ).
                    append("\nWith options: ").
                    append( options);
        }
